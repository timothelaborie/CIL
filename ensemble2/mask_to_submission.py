...
flags.DEFINE_string(
    "submission_filename", cwd+"/ensemble2/submission_"+name+".csv", "The output csv for the submission.")
flags.DEFINE_string(
    "base_dir", cwd+"/data/ethz-cil-road-segmentation-2023/test/pred_"+name, "The directory with the predicted masks.")
...
def mask_to_submission_strings(image_filename, mask_dir=None):
    ...
    loc = image_filename.replace(cwd+"/data/ethz-cil-road-segmentation-2023/test/pred_"+name, "")
    img_number = int(re.search(r"\d+", loc).group(0))
    # print(loc)
